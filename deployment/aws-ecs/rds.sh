#!/usr/bin/env bash
# RDS helpers: find existing DB or create a new one. Persist instance id to $DB_INSTANCE_FILE
...
find_or_create_rds_instance() {
    ...
    local existing_dbs=("ecommerce-lambda-db" "ecommerce-fargate-db" "ecommerce-ec2-db")
    local found_db=""
